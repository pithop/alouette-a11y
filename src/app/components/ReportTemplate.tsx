...
// Helper to group violations by their RGAA mapping
  ...
  return (
    <html lang="fr">
      <head>
        <meta charSet="UTF-8" />
        <style>{`
          body { font-family: Arial, sans-serif; margin: 0; padding: 0; background-color: #f9fafb; color: #1f2937; }
          ...
          .score { font-size: 72px; font-weight: bold; color: ${score >= 80 ? '#16a34a' : score >= 50 ? '#f97316' : '#dc2626'}; }
          ...
          .violation-item { padding: 12px 16px; border-top: 1px solid #e5e7eb; }
          .violation-desc { font-weight: 500; }
          .violation-impact { font-size: 14px; color: #ef4444; margin-top: 4px; }
      ...
      <body>
        <div className="container">
          <div className="header">
            ...
            <p style={{ marginTop: '8px', color: '#6b7280' }}>Pour le site : <a href={siteUrl}>{siteUrl}</a></p>
          ...
          <div className="summary">
            ...
            <p style={{ marginTop: '16px' }}>Total de <strong>{results.totalViolations}</strong> problèmes détectés.</p>
          </div>
          ...
          {Object.entries(grouped).map(([rgaa, group]) => (
            <div key={rgaa} className="criterion-group">
              <div className="criterion-header">
                [RGAA {rgaa}] {group.name} ({group.violations.length} instances)
              </div>
        ...
        <div className="footer">
          Rapport généré par Alouette A11Y. Cet audit automatisé ne remplace pas un audit manuel complet.
        </div>
      </body>
    </html>
  );
